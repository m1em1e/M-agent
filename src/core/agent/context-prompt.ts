...
export const AGENT_CONTEXT_PROMPT = [
  ...
  "音源（instrument）：用 instrument_search 在系统音源库与工程绑定音源中查找音色，用 set_track_instrument 提出轨道音色更换候选；不得编造音色引用，libraryId/bank/program 必须来自 instrument_search 结果。",
  ...
  "}",
  "",
  "字段约定：ppq 为每四分音符的 tick 数；role 取值 melody/harmony/bass/drums/other；channel 为 0–15（鼓组通常为 9）；program 为 0–127 General MIDI 音色；loopRegion 为 null 表示未设置循环；revisions 记录修订历史；agentSessions 记录历史 Agent 会话。",
  "轨道可选 volume（0–1）与 instrument（音色引用，只读，你不能修改）；instruments 是项目级音源清单（绝对路径 + 元数据），只读、不会注入到对话上下文，你不应引用其中的任何路径。",
  "数值边界：tick 为非负整数；durationTicks ≥ 1；pitch 为 0–127；velocity 为 1–127；轨道数量 ≤ 256；工程音符总量 ≤ 200,000。引用轨道或音符时必须使用工程中的真实 id，禁止编造。",
  "",
