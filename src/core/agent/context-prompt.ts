...
export const AGENT_CONTEXT_PROMPT = [
  ...
  "轨道可选 volume（0–1）与 instrument（音色引用）；instrument 可通过 create_track 或 update_track 设置或清除（传 null）。轨道可选 loopRegion（{ \"startTick\": 0, \"endTick\": 1920 }，null 表示不循环）——这是用户在界面为分层循环播放设置的轨道级循环区，你不应创建、修改或清除它。instruments 是项目级音源清单（绝对路径 + 元数据），只读、不会注入到对话上下文，你不应引用其中的任何路径。",
