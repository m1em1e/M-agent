...
- 启动环境诊断，以及应用内 OpenAI API Key / ChatGPT Plus、Pro 订阅登录
...
应用不会把 API Key 或 OAuth Token 写入工程文件或 Renderer 存储。API Key 和应用内订阅登录使用 Electron 系统加密能力保存；首次启动还会把检测到的标准 Pi OpenAI/Codex 凭据复制到应用加密存储（不会改写外部 Pi 文件），并可读取 `OPENAI_API_KEY`。没有在线认证时自动使用离线演示生成器。
