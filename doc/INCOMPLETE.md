...
5. **真实云端 Provider 尚未验证**：仅自动验证 Pi 离线 Provider。未用真实 API Key 验证 OpenAI Responses 请求与工具调用、认证失败/限流/网络错误/中止、实际 Token/费用/输出质量、在线三模式行为；ChatGPT Plus/Pro OAuth 已接入但未发起真实请求验证 entitlement。
...
9. **优化 Skill 调用**：提升 Skill 嵌套调用的稳定性与效率。待办：梳理 `invoke_skill` 的上下文注入、递归预算与结果合并链路，评估子 Skill 复用父级模型/会话上下文以减少重复调用，优化失败重试与降级策略；补充嵌套调用端到端测试。
