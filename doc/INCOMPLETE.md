...
### 发布与打包验证
- Electron 升级到 43.4.0 后 Windows/macOS/Linux 打包链路需重新验证；NSIS 安装、首次启动、升级覆盖和卸载未验证。
- 无 npm/Node/Pi CLI 的干净 Windows 环境验证未做。
- 代码签名（Windows/macOS 公证）与三端发布验证未完成。
- `package:mac` / `package:linux` 产物需在对应系统或 CI 验证。
...
WAV/OGG 离线渲染导出尚在方案阶段（未实施），见 [AUDIO_EXPORT.md](AUDIO_EXPORT.md)。
